...
class SchedulerEngine:
    ...
    def _ensure_event_backed_schedules(self) -> None:
        ...
        if existing:
            for row in existing:
                event = self.events.record_event(
                    "schedule.created", source.id, self._schedule_payload_from_row(row),
        ...
        for spec in create_default_lab_schedules():
            sid = generate_id("SCH-")
            payload = {
                "id": sid, "name": spec["name"], "trigger_type": spec["trigger_type"],
                "trigger_spec": spec["trigger_spec"], "agent_role": spec["agent_role"],
                "prompt_template": spec["prompt_template"], "enabled": True,
                "catch_up_policy": spec.get("catch_up_policy", "run_once"),
                "autonomy_policy": spec["autonomy_policy"], "last_run_at": None,
                "next_run_at": None, "created_at": now,
            }
            event = self.events.record_event(
                "schedule.created", source.id, payload,
    ...
    def _migrate_legacy_default_schedules(self, source_id: str) -> None:
        ...
        if not row:
            return
        trigger_spec = json.loads(row["trigger_spec_json"])
        if row["trigger_type"] != _LEGACY_ADVISOR_PREP_TRIGGER["trigger_type"] or trigger_spec != _LEGACY_ADVISOR_PREP_TRIGGER["trigger_spec"]:
            return
        ...
        event = self.events.record_event(
            "schedule.created", source_id, payload,
    ...
    def _relative_meeting_due(self, row: Any, current: datetime) -> Optional[dict[str, Any]]:
        ...
        if explicit:
            ...
            candidates.append((explicit_at, {
                "meeting_id": explicit["id"], "meeting_kind": explicit["kind"],
                "meeting_title": explicit["title"], "scheduled_at": explicit["scheduled_at"],
                "recurring": False,
            }))
        ...
        if meeting_kind == "advisor":
            ...
            if weekly:
                ...
                candidates.append((occurrence, {
                    ...
                    "meeting_kind": "advisor", "meeting_title": "Weekly Advisor Meeting",
                    "scheduled_at": occurrence.isoformat(), "recurring": True,
                    "weekly_spec": weekly,
                }))
    ...
    def _weekly_cron_due(self, row: Any, current: datetime) -> Optional[dict[str, Any]]:
        ...
        if weekday_name not in _WEEKDAYS:
            return None
        hour = int(spec.get("hour", 0)); minute = int(spec.get("minute", 0))
        ...
        occurrence_local = datetime(occurrence_date.year, occurrence_date.month, occurrence_date.day, hour, minute, tzinfo=zone)
    ...
    def _event_due(self, row: Any, current: datetime) -> Optional[dict[str, Any]]:
        ...
        event = self.db.fetchone(
            """SELECT * FROM events WHERE event_type = ? AND occurred_at > ? AND occurred_at <= ?
    ...
    def mark_triggered(self, schedule_id: str, executed_at: datetime, *, trigger_at: Optional[str] = None) -> None:
        ...
        source = self.events.register_source("scheduler", "Master Scheduler", "master-os-scheduler")
        event = self.events.record_event(
            "schedule.triggered", source.id,
            {"id": schedule_id, "last_run_at": executed_iso, "trigger_at": occurrence},
            occurred_at=executed_iso, dedup_key=f"schedule-trigger:{schedule_id}:{occurrence}",
        )
        apply_event(self.db, event)
    ...
    def trigger_routine(self, schedule_name: str) -> dict[str, Any]:
        ...
        role = row["agent_role"]
        if role != "critic":
            raise RuntimeError(f"Schedule {schedule_name!r} requires a configured {role!r} runtime handler; refusing a fake success.")
        report = self.critic.evaluate_health()
        now = datetime.now(timezone.utc)
        result: dict[str, Any] = {
            "schedule": schedule_name, "executed_at": now.isoformat(), "status": "ok",
            "health_report": {"velocity": report.research_velocity, "warning": report.fake_progress_warning,
                              "message": report.warning_message, "burn_warnings": report.resource_burn_warnings},
        }
